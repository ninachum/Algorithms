/******************************************************************************
 *  Compilation:  javac myTrieSET.java
 *  Execution:    java myTrieSET < words.txt
...
/**
 *  The {@code myTrieSET} class represents an ordered set of strings over
...
public class myTrieSET implements Iterable<String> {
    private static final int R = 26;        // extended ASCII -------------------- I MODIFIED : R = 26 (ENGLISH ALPHABET SIZE), so, this trie is a 26-way trie.
	...
	*/
    ...
    // modified part.
    /*
    public boolean isThereKeysWithPrefix(String key) {
        if (key == null) throw new IllegalArgumentException("argument to isThereKeysWithPrefix() is null");
    	Node x = get(root, key, 0);
    	if (x == null) return false;
    	if (x.isString == true)
    		return true;
	...
	*/
    ...
    public boolean isThisNodeHasString(Node x) {
        if (x == null) return false;//throw new IllegalArgumentException("argument to isThisNodeOK() is null");
        if (x.isString) return true;
        return false;
    }
    
    public boolean needToGoFurther(Node x) {
        if (x == null) return false;//throw new IllegalArgumentException("argument to needToGoFurther() is null");
        for (int i = 0; i < R; i++) {
        	if (x.next[i] != null)
        		return true;
        }
    ...
    public Node nextNode(Node x, char c) {
        if (x == null) throw new IllegalArgumentException("Node argument to needToGoFurther() is null");
    ...
    /**
     * Unit tests the {@code myTrieSET} data type.
     *
