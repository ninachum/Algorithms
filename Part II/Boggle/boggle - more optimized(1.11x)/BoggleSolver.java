...
public class BoggleSolver {
	...
	public Iterable<String> getAllValidWords(BoggleBoard board) {
		...
		boardContent = new char[boardRows][boardCols]; 
		
		for (int i = 0; i < boardRows; i++) {
			for (int j = 0; j < boardCols; j++) {
				boardContent[i][j] = board.getLetter(i, j);
			}
		}
		
		// initialize adjacency matrix for each dice.
	...
	private void collect(BoggleBoard board, int i, int j, PatriciaSET coll, boolean[][] marked, StringBuilder sb, Node curr, Object[][] adjMat) {
		...
		if (boardContent[i][j] == 'Q')
			sb.append("QU");
		else
			sb.append(boardContent[i][j]);
		...
		// for valid child of valid current Node, do recursive calls. - remember : curr is changed during collect().
		if (curr != null) {
